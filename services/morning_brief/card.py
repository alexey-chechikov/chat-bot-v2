...
def build_card(data: dict) -> str:
    ...
    if r:
        ...
        # Риск-тилт для динамиков (2026-07-14, бэктест 2г): направленного дрейфа
        # по зоне НЕТ (нельзя «увеличь шорты»), но глубокие хвосты −5%/24ч под
        # красной в ×1.8 чаще (2.8% vs 1.5% баров). Под красной = защита лонг-ног.
        if "SHORT" in r["zone"]:
            L.append("   ⚖️ риск-тилт: под красной хвост-риск ×1.8 — лонг-ноги динамиков легче/капнуть")
        elif "LONG" in r["zone"]:
            L.append("   ⚖️ риск-тилт: над красной — симметрия ног ок, хвост-асимметрии нет")
